...
const extPath = path.join(__dirname, '..', 'plugins/vue3-devtool');
  ...
  if (process.type === 'renderer' || process.type === 'browser') {
    ...
    if (process.type === 'renderer') {
      if (
        ...
        electron.remote.BrowserWindow.getDevToolsExtensions()[extName]
      )
        return true;
    ...
    if (process.type === 'browser') {
      if (electron.BrowserWindow.getDevToolsExtensions && electron.BrowserWindow.getDevToolsExtensions()[extName])
        return true;
